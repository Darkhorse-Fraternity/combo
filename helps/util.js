...
export function shadeBlend(p, c0, c1) {
  const n = p < 0 ? p * -1 : p; const u = Math.round; const
    w = parseInt;
  if (c0.length > 7) {
    const f = c0.split(','); const t = (c1 || (p < 0 ? 'rgb(0,0,0)' : 'rgb(255,255,255)')).split(',');
    const R = w(f[0].slice(4)); const G = w(f[1]); const
      B = w(f[2]);
    return `rgb(${u((w(t[0].slice(4)) - R) * n) + R},${u((w(t[1]) - G) * n) + G},${u((w(t[2]) - B) * n) + B})`;
  }
  const f = w(c0.slice(1), 16); const t = w((c1 || (p < 0 ? '#000000' : '#FFFFFF')).slice(1), 16);
  const R1 = f >> 16; const G1 = f >> 8 & 0x00FF; const
    B1 = f & 0x0000FF;
