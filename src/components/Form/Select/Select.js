...
class Select extends Component {
    ...
    toggleSelector(options) {
        if(options && options.length){
            const items = options.map(op=>op[this.props.labelKey])
            showSelector(items,(res)=>{
                if(res !==items.length){
                    this.onValueChange(options[res][this.props.valueKey])
                }
            })
        }
    ...
    render() {
        ...
        const labelsByValue = options.reduce((carry, option) => {
            carry[option[valueKey]] = option[labelKey]
            return carry
        }, {})
        ...
        return (
            <SelectWrapper style={style} inlineLabel={inlineLabel} theme={theme}>
                {/*<Modal*/}
                ...
                {/*>*/}
                    ...
                    {/*</Picker>*/}
                {/*</Modal>*/}
                <TouchableOpacity onPress={()=>this.toggleSelector(options)}>
