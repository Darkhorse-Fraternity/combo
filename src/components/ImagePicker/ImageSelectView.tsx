...
interface PropsType {
  ...
  files: any;
  onChange: () => void;
  onSelectState: (state: 'start' | 'stop') => void
}


export default class ImageSelectView extends PureComponent<PropsType, any> {
  ...
  _selectImage = () => {
    ...
    onSelectState && onSelectState('start')
    imagePicker(opt, (response) => {
      onSelectState && onSelectState('stop')
