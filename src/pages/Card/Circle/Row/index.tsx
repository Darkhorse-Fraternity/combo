...
const RenderChatBtn: FC<ItemType> = (item) => {
  const { commentNew, commentNum, objectId } = item;
  ...
  return (
    <StyledChatbtn
      ...
      <StyledMaterialCommunityIcons
        ...
        color="#8c8c85"
      />
      {commentNew && item.user.objectId === user?.objectId && <StyledNewTip />}
  ...
  render() {
    ...
    return (
      <StyledButton>
        ...
        <StyledBottom>
          <StyledDateView>
            <StyledDateText>{fromNow}</StyledDateText>
            <StyledBottomBtnView>
              <RenderLikes {...item} />
              <RenderChatBtn {...item} />
            </StyledBottomBtnView>
