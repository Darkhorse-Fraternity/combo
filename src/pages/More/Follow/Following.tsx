    ...
    follow: (isExist, num) => {
      dispatch((dispatch, getState) => {
        ...
        if (isExist) {
          ...
          dispatch(
            reqChangeData(FRIENDNUM + beFollowedUserId, {
          ...
          selfNum > 0 &&
            dispatch(
              reqChangeData(FRIENDNUM + userId, {
...
export default class Following extends PureComponent {
  ...
  _renderHeader = () => {
    ...
    const friendeExist =
      this.props.friendeExist && this.props.friendeExist.toJS();
    let isFollow = true;
    let load = true;
    if (friendeExist) {
      ...
      <StyleHeader>
        <StyleHeaderInner>
          <StyleHeaderInnerLeft>
            ...
            {this._renderFollow(data, followees_count, followers_count)}
          </StyleHeaderInnerLeft>
          <StyleHeaderInnerRight>
            <View
              style={{
        ...
        <LCList
          ...
          dataMap={(data) => {
            const results = data.results.filter(
              (item) => item.iCard.state >= CircleState.open,
            );
            return { results };
          }}
          reqParam={param}
